...
export async function createMagicToken(email, next, minutes = 7 * 24 * 60) {
...
// Short-lived token embedded in the admin "Approve access" email.
export async function createApproveToken(email, days = 7) {
  return await new SignJWT({ sub: email, purpose: 'approve' })
    .setProtectedHeader({ alg: 'HS256' })
    .setIssuedAt()
    .setExpirationTime(`${days}d`)
    .sign(secretKey());
...
export function sessionCookie(value, days = 30) {
  ...
  return `${SESSION_COOKIE}=${value}; Path=/${domainPart}; HttpOnly; Secure; SameSite=Lax; Max-Age=${maxAge}`;
}

export function clearCookie() {
  const domain = process.env.COOKIE_DOMAIN;
  const domainPart = domain ? `; Domain=${domain}` : '';
  return `${SESSION_COOKIE}=; Path=/${domainPart}; HttpOnly; Secure; SameSite=Lax; Max-Age=0`;
}
